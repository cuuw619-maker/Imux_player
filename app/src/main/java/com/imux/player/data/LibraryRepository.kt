...
class LibraryRepository(
 ...
 suspend fun scanAll()=withContext(Dispatchers.IO){
  folders.first().forEach{scan(it.uri)}
 }

 private suspend fun scan(uri:String){
  val root=DocumentFile.fromTreeUri(context,android.net.Uri.parse(uri))?:return
   ...
   directory.listFiles().forEach{file->
    if(file.isDirectory){
     ...
     out+=Track(
      ...
      mime=context.contentResolver.getType(file.uri).orEmpty(),
      size=file.length()
     )
    }
   }
